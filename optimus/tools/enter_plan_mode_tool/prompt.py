"""enter_plan_mode_tool/prompt.py — port of EnterPlanModeTool/prompt.ts.

External variant only (USER_TYPE !== 'ant'); the Ant variant is omitted per the
project's acceptable-omission rules. feature('KAIROS')/interview-phase gating
compiles to False, so the "What Happens in Plan Mode" section is always present
(matching the non-feature-gated external flavor).
"""
from __future__ import annotations

from optimus.tools.ask_user_question_tool.prompt import ASK_USER_QUESTION_TOOL_NAME
...
def get_enter_plan_mode_tool_prompt() -> str:
    # feature('PLAN_MODE_V2_INTERVIEW') → False: whatHappens is always included.
    what_happens = WHAT_HAPPENS_SECTION
